...
for /F "eol=# tokens=* delims=" %%a in (%~dp0\default-config.env %~dp0\default-config-mono.env) do (
    if not "%%a"=="" set "%%a"
)
...
docker build -t topicmanagementservice-mono -f %~dp0\docker\mono\Dockerfile.mono .

echo Deploying container...
docker stack deploy -c %~dp0\docker\mono\docker-compose-mono.yml topicmanagementstack